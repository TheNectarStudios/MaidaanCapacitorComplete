...
export const CategoriesRoundContainer = ({
	...
	return (
		<div
			className={`flex flex-col items-center justify-start bg-[#3a3a3a] h-full ${responseView ? "w-[100%]" : "min-h-[100vh] w-[100vw]"
				}`}>
			{!responseView && (
			...
			<Box>
				<div className="grid mb-1 mt-1">
					{correctlyMatchedArray?.map((question) => (
						<div
							className={`flex flex-col items-center justify-center ${correctCategoriesAnswered?.includes(question[0].category)
								? "bg-[#ccf900]"
								: "bg-white"
			...
			{!responseView && !showCorrectAnswer && (
				<span className="text-xl font-bold text-white mt-2">
					{`${correctlyMatchedArray?.length || 0} ${correctlyMatchedArray?.length === 1 ? "Group" : "Groups"
