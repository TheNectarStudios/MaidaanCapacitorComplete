...
import { db, firebaseAuth, database } from "../../../firebase-config";
...
const Lobby = (props) => {
  ...
  const handleLogout = async () => {
    ...
    //update the status of the user to offline in the realtime database..
    await set(ref(database, `/status/${user?.id}`), "offline");
  ...
  const getRegisteredTournaments = async (tournamentList) => {
    ...
    if (tenantSwitchLoading) {
      ...
      if (ongTour.length || isUpcomingTourStartingInLessThan24Hours) {
        const isAnyOngoingTournamentTenant = ongTour.some((t) =>
          t.tenantIds?.includes(nonDefaultTenantId)
        );
        const isAnyUpcomingTournamentTenant = upcomTour.some((t) =>
          t.tenantIds?.includes(nonDefaultTenantId)
        );
        if (isAnyOngoingTournamentTenant || isAnyUpcomingTournamentTenant) {
            ...
            <div className="mx-[10px] mt-8">
              ...
              <div className="md:flex md:justify-center">
                <div className="max-w-[578px] w-full h-full text-xl shadow-[rgba(0,0,0,0.35)_0px_5px_15px] bg-[rgba(58,58,58,0.9)]">
                  <div
                  className="bg-cover banner-card"
                    style={{
          ...
          <>
            <div className="mx-[10px] mt-8">
              ...
              <div className="md:flex md:justify-center">
                <div className="max-w-[578px] w-full h-full text-xl shadow-[rgba(0,0,0,0.35)_0px_5px_15px] bg-[rgba(58,58,58,0.9)]">
                  <div
                    className="w-full banner-card bg-cover"
                    style={{
